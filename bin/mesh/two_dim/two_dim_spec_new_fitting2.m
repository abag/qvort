function two_dim_spec_new_fitting2(filenumbers)
do_fit=1;
cutoff=0;
...
if length(filenumbers)>=1
  ...
  if do_fit==1
    ...
    disp(sprintf('creating a fit from just after k_intervortex to k_resolution'))
    cfit1 = polyfit(log(k(k_intervortex+8:k_resolution)),log(avg_spect(k_intervortex+8:k_resolution)),1);
    dummy_spect1=k.^(cfit1(1));
    disp(sprintf('the fit values are: '))
    cfit1
    scaling_factor1=0.8*sum(avg_spect(1:10))/sum(dummy_spect1(1:10));
    dummy_spect1=dummy_spect1*scaling_factor1;
    disp(sprintf('creating a fit from just before k_intervortex to just after k_intervortex'))
    cfit2 = polyfit(log(k(k_intervortex-10:k_intervortex+8)),log(avg_spect(k_intervortex-10:k_intervortex+8)),1);
    dummy_spect2=k.^(cfit2(1));
    disp(sprintf('the fit values are: '))
    cfit2
    scaling_factor2=0.8*sum(avg_spect(1:10))/sum(dummy_spect2(1:10));
    dummy_spect2=dummy_spect2*scaling_factor2;
    dummy_spect3=k.^(-5/3);
    scaling_factor3=0.8*sum(avg_spect(1:10))/sum(dummy_spect3(1:10));
    dummy_spect3=dummy_spect3*scaling_factor3;
    hold on
    loglog(k(k_intervortex+8:k_resolution),dummy_spect1(k_intervortex+8:k_resolution),'--r','LineWidth',2)
    hold on
    loglog(k(k_intervortex-10:k_intervortex+8),dummy_spect2(k_intervortex-10:k_intervortex+8),'--b','LineWidth',2)
    hold on
    loglog(k(k_box:k_intervortex-10),dummy_spect3(k_box:k_intervortex-10),'--g','LineWidth',2)
  end
  xlabel('log k','FontSize',16) ; ylabel('log E(k)','FontSize',16)
  set(gca,'FontSize',16)
  figure('Name','compensated avgerage E(k)')
  loglog(k(k_intervortex+8:k_resolution),(k(k_intervortex+8:k_resolution).^(-cfit1(1))).*avg_spect(k_intervortex+8:k_resolution),'Color','r','LineWidth',1.5)
  hold on
  loglog(k(k_intervortex-10:k_intervortex+8),(k(k_intervortex-10:k_intervortex+8).^(-cfit2(1))).*avg_spect(k_intervortex-10:k_intervortex+8),'Color','b','LineWidth',1.5)
end
