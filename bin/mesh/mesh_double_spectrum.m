function mesh_double_spectrum(ux,uy,uz,unormx,unormy,unormz,n)
...
for i=1:n
    for j=1:n
        for k=1:n
            ...
            r=round(sqrt(ii^2+jj^2+kk^2));
            if r==0 ; continue ; end
            spectn(r)=spectn(r)+energyr(i,j,k)+energyi(i,j,k);
        end
    end
end
scaling_factor=spectn(3)/dummy_spect(3);
spectn=spectn/scaling_factor;
hold on
k=(1:midpt)*(2*pi/dims(2));
loglog(k,spectn(1:midpt),'b','LineWidth',2)
figure('Name','compo')
loglog(k,spect(1:midpt).*k.^(5/3),k,spectn(1:midpt).*k.^(5/3),k,dummy_spect(1:midpt).*k.^(5/3))
