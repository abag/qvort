function mesh_long_structure_with_errorplot(filenumbers,power,fit,varargin)
...
for ifile=filenumbers
    ...
    S2(1:(msize/2),fcounter)=0;
    for k=1:msize
        for j=1:msize
            for i=1:(msize/2)
                for r=1:(msize/2)
                    S2(r,fcounter)=S2(r,fcounter)+(abs((vel_x(k,j,i+r)-vel_x(k,j,i)))^pow);
                end
            end
        end
    end
    for k=1:msize
        for i=1:msize
            for j=1:(msize/2)
                for r=1:(msize/2)
                    S2(r,fcounter)=S2(r,fcounter)+(abs((vel_y(k,j+r,i)-vel_y(k,j,i)))^pow);
                end
            end
        end
    end
    for i=1:msize
        for j=1:msize
            for k=1:(msize/2)
                for r=1:(msize/2)
                    S2(r,fcounter)=S2(r,fcounter)+(abs((vel_z(k+r,j,i)-vel_z(k,j,i)))^pow);
                end
            end
        end
    end
end
S2(:,fcounter)=S2(:,fcounter)./(number_of_files*3*msize*msize*msize/2);
save struture.mat S2
figure('Name',strcat('2nd order long struct func., fluid:',fluid));
loglog(1:msize/2,mean(S2'),'LineWidth',2)
xlabel('r','FontSize',14)
ylabel('struct. func.','FontSize',14)
set(gca,'Fontsize',14)
if do_fit == 1
    disp(sprintf('creating a fit'))
    cfit = polyfit(log(1:round(msize/3)),log(S2(1:round(msize/3),fcounter)'),1);
    cfit
    hold on
    loglog((1:msize/3),exp(abs(cfit(2))).*(1:msize/3).^(cfit(1)),'--g','LineWidth',2);
    hold on
    loglog((1:msize/2),exp(abs(cfit(2))).*(1:msize/2).^(2/3),'--r','LineWidth',2);
    hold off
end
print -depsc structure_func_with_fit.eps
figure('Name',strcat('error plot longitudinal struct func., fluid:',fluid));
errorbar(1:msize/2,mean(S2'),std(S2'),'r')
set(gca,'xscale','log','yscale','log')
print -depsc error_plot.eps
