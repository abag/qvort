...
rho=log(rho);
range=ceil(max(rho)-min(rho))+1;
cmap=colormap(jet(range));
store_caxis=([min(rho) max(rho)]);
for i=1:number_of_particles
  plot3(x(i),y(i),z(i),'o','MarkerEdgeColor','k','MarkerFaceColor',cmap(1+ceil(rho(i)-min(rho)),:),'MarkerSize',4)
  %plot3(x(i),y(i),z(i),'o','MarkerEdgeColor','r','MarkerFaceColor','r','MarkerSize',6)
  hold on
end
