...
%
%   SLICER is an graphical interface to explore slice-by-slice 3D images.
%   Index of current slice is given under the slider, and cursor position
%   is indicated when mouse is moved over image.
%   SLICER should work with any kind of 3D images (binary, gray scale and
%   8 bits RGB images)
%
%   slicer(IMG)
%   where IMG is a preloaded M*N*P matrix, opens slicer GUI.
...
%
%   slicer(IMGNAME, ...)
%   Load the stack specified by IMGNAME. It can be either a tif bundle, or
%   the first file of a series.
%   Requires the function 'readstack' to work.
%
%   slicer
...
% Last Modified by GUIDE v2.5 10-Nov-2010 12:25:40
...
if isempty(varargin)
    ...
    img = readstack(fullfile(pathName, fileName));
    ...
    elseif ischar(var)
        % if a character string is given, try to load from an imag file
        imgName = var;
        img = readstack(imgName);
        handles.imgName = imgName;
        % TODO: call a specific function to load image, 
        % in order to manage more file formats
        
    elseif isa(var, 'Image')
        % Try to interpret as Image object (not included in default slicer)
        if getDimension(var)~=3
        ...
        calib = var.getSpatialCalibration();
        if ~isempty(calib)
            % Calibration info, in user unit, in xyz order
            handles.voxelOrigin     = calib.origin;
            handles.voxelSize       = calib.spacing;
            handles.voxelSizeUnit   = calib.unitName;
        end
    else
...
% TODO: use a imageType field, that can be 'gray', 'color' or 'vector'.
handles.color   = false;
...
% show middle of image by default
handles.slice = round(dim(3)/2);

% Parses other input arguments
handles = parseInputOptions(handles, varargin{:});

% TODO: call setupImage somewhere

% setup initial zoom: round zoom to the closest lower power of 2.
    ...
    switch lower(param)
        ...
        case 'zoom'
            ...
            if zoom>handles.zoomMin && zoom<handles.zoomMax
                ...
                disp('slicer: bad value for parameter zoom');
            end
        case 'spacing'
...
function data = getDisplayData(handles)
...
function setImageLUT(hObject, eventdata, handles, lutName)
...
function [mini maxi] = computeExtremeValues(img)
...
%fprintf(sliderPattern, 'Y', ylim(1), ymin, ymax);
...
% --------------------------------------------------------------------
function menuFiles_Callback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function itemOpen_Callback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function itemImportDicom_Callback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function itemImportAnalyze_Callback(hObject, eventdata, handles)
...
info = analyze75info(fullfile(pathname, filename));
...
% --------------------------------------------------------------------
function itemImportInterfile_Callback(hObject, eventdata, handles)
...
info = interfileinfo(fullfile(pathname, filename));
...
% --------------------------------------------------------------------
function itemImportMetaImage_Callback(hObject, eventdata, handles)
% hObject    handle to itemImportMetaImage (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

[filename, pathname] = uigetfile( ...
       {'*.mhd', 'MetaImage data file(*.mhd)'; ...
        '*.*',   'All Files (*.*)'}, ...
...
info = metaImageInfo(fullfile(pathname, filename));
...
% --------------------------------------------------------------------
function itemImportRawData_Callback(hObject, eventdata, handles)
...
dims = [0 0 0];
for i=1:3
    dims(i) = str2num(answers{i}); %#ok<ST2NM>
end
...
% read raw stack (use corerction of some bugs in 'readstack' function)
...
% Only handle.img is valid. This function set up other fields from the
...
% --------------------------------------------------------------------
function itemQuit_Callback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function menuChangeLUT_Callback(hObject, eventdata, handles)
% hObject    handle to menuChangeLUT (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% --------------------------------------------------------------------
function itemDisplayImageInfo_Callback(hObject, eventdata, handles)
...
pos = get(f, 'position');
width = pos(3);
% sum iof width is not equal to 1 to avoid rounding errors.
...
% --------------------------------------------------------------------
function itemChangeVoxelSize_Callback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function menuGrayscaleRange_Callback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function itemGrayRangeImage_Callback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function itemGrayRangeDataType_Callback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function itemGrayRangeManual_Callback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function itemRotateImageLeft_Callback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function itemRotateImageRight_Callback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function itemRotateImageXUp_Callback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function itemRotateImageXDown_Callback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function itemRotateImageYLeft_Callback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function itemRotateImageYRight_Callback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function itemFlipImageX_Callback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function itemFlipImageY_Callback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function itemFlipImageZ_Callback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function itemZoomOut_Callback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function itemZoomOne_Callback(hObject, eventdata, handles) %#ok<*INUSD>
...
% --------------------------------------------------------------------
function itemZoomBest_Callback(hObject, eventdata, handles) %#ok<*DEFNU>
...
% --------------------------------------------------------------------
function itemViewHistogram_Callback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function itemAbout_Callback(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function imageDisplay_ButtonDownFcn(hObject, eventdata, handles) %#ok<*INUSL>
...
function imageDisplay_ButtonMotionFcn(hObject, eventdata, handles) %#ok<*INUSL>
% Update display of mouse coordinate and pixel value
...
% Display coordinate of clicked point
if sum(handles.voxelSize~=1)>0
    ...
    locString = sprintf('x=%d y=%d', coord(1), coord(2));
end
...
if handles.color
    ...
    valueString = sprintf('R=%03d G=%03d B=%03d', ...
        rgb(1), rgb(2), rgb(3));
...
elseif handles.vector
    ...
    valueString = sprintf('value=%7.3f', norm);
    ...
    if ~isfloat(value)
        ...
        valueString = sprintf('value=%7.3f', value);
    end
end
set(handles.pointValueText, 'string', valueString);


function point = displayToUserPoint(handles, point)
